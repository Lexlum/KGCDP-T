...
def collate_fn_graph_text(batch):
    nodes, edges, types, outputs, pointer, pairs, relations, positions, \
    ...
    relations, relations_masks = padding(relations, pad_idx=0)
    ...
    heads, tri_mask = list_padding(heads, pad_list=0)
    tails, _ = list_padding(tails, pad_list=0)

    return nodes, edges, types, node_masks, descriptions, description_masks, positions, relations, relations_masks, pairs, pair_masks, \
           outputs, output_masks, pointer, pointer_masks, heads, tails, tri_mask


def list_padding(inputs, pad_list):
    lengths = [len(inp) for inp in inputs]
    max_len = max(lengths)
    padded_inputs = [inp + [[pad_list]] * (max_len - len(inp)) for inp in inputs]
