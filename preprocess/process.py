...
bert_tokenizer = BartTokenizer.from_pretrained('../pretrained_model/bart-base')
bart_tokenizer = BartTokenizer.from_pretrained('../pretrained_model/bart-base')
...
for fn in filename:
    ...
    for d in data:
        ...
        new_dict['plm_output'] = bart_tokenizer.convert_tokens_to_ids(bart_tokenizer.tokenize(new_dict['target_txt']))
        ...
        assert bart_tokenizer.convert_tokens_to_ids(test_output) == new_dict['plm_output'],\
            "The test output and plm output are not equal!"
        ...
        for n in new_dict["split_nodes"]:
            entities.add(bert_tokenizer.convert_tokens_to_ids(n))
        ...
        triples = new_dict["triples"]
        triples_h = [bert_tokenizer.tokenize(tri[0]) for tri in triples]
        triples_t = [bert_tokenizer.tokenize(tri[2]) for tri in triples]
        new_dict["heads"] = triples_h
        new_dict["tails"] = triples_t
...
entities = [bert_tokenizer.convert_tokens_to_ids("<pad>")] + list(entities)

nodeidx2bartidx = dict()
for idx, en in enumerate(entities):
    nodeidx2bartidx[idx] = en
pickle.dump(nodeidx2bartidx, open("nodeidx2bartidx.pkl", "wb"))
